...
# 20 = Nilai Math A - Nilai Math B
# 0.12296 = Log of Odds buat Math terhadap honors
...
- Semakin banyak predictor, semaik sedikit value AIC dan residual deviance
- AIC bisa dipakai untuk mebandingkan model mana yang terbaik jika n banyak model memiliki jumlah predictor yang sama
-
...
Ans: No
- AIC dan residual deviance sangat kecil dari standarnya
- Ada predictor yang "Terlalu" dominan daripada yang lain.
...
Jika ada predictor yang "log of odds"nya lebih dari 10 atau kurang dari -10 -> perfect separation
...
Jika `log_inc` dan `annual_inc` dibuatkan sebagai predictor pada waktu yang sama, terjadi multicolinearity.
...
- Train: Data untuk buat model
- Test: Data untuk menguji akurasi model (boleh tidak acak)
- Evaluation: Data untuk menguji akurasi model (**harus** acak)
...
Note:
- Paling signifikan: installment
- Menurunkan resiko bayar: 'annual_inc' dan 'gradeCtoA'
- Base model: nasabah yang purposenya 'credit card' dan ownershipnya 'mortgage'
...
- "Response": Output berupa probability
- "Link": Output berupa 'Log of Odds'
...
Notes:
* Actual == Prediction (P) -> True Positive
* Actual == Prediction (N) -> True Negative
* Actual != Prediction (P) -> False Positive
* Actual != Prediction (N) -> False Negative
